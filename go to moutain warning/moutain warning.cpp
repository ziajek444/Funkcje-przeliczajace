...
double ZagrorzenieCal(double Temperatura, double SilaWiatru, double Cisnienie, double Wilgotnosc, bool debug = 0)
{
    ...
    // wzÛr -> zagrozenie = ( 50000SW * sqrt(Wilgotnosc) ) / ( Temperatura^2 * (cisnienie/10) )
...
int main()
{
    double Temperatura; // Mniej niø 15 -> èLE
    double SilaWiatru; // Wiecej niz 20m/s -> èLE
    // kierunek wiatru nie ma znaczenia dla zagraoøenia
    double Cisnienie; // Mniej niz 700 Hpa -> èLE
    double Wilgotnosc; // Mniej niz 30% èLE AND WiÍcej niø 80% -> èLE
    ...
    // jezeli jest mniejsze lub rÛnw 1 to ok :d
